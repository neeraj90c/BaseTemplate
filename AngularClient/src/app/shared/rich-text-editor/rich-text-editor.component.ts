...
export class RichTextEditorComponent implements OnInit {
  RTE!: any
  @Input({ required: true }) control!: FormControl; // Receive FormControl from parent
  ...
  ngOnInit(): void {
    if (this.control) {
      this.control.valueChanges.subscribe(value => {
        if (this.RTE && value !== this.RTE.root.innerHTML) {
          let safeHtml = this.sanitizer.bypassSecurityTrustHtml(value);
          this.RTE.setHTML(safeHtml)
  ...
  initializeRTE() {
    this.RTE = new RichTextEditor('#rtediv')
    console.log(this.RTE.destroy);
  ...
  ngAfterViewInit() {
    ...
    if (this.control?.value) {
      this.RTE.setHTML(this.control.value) // Initialize with form value
    }
  }
  ...
  ngOnDestroy() {
    if (this.RTE && typeof this.RTE.destroy === 'function') {
      this.RTE.destroy(); // Clean up editor resources
    } else {

      const floatPanels = document.querySelectorAll('.rte-floatpanel-paragraphop');
      floatPanels.forEach(el => el.remove());
      this.RTE = null;
    }
  }
}
