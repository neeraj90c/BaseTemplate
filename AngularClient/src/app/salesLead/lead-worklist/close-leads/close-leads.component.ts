...
@Component({
  selector: 'app-close-leads',
  templateUrl: './close-leads.component.html',
  styleUrls: ['./close-leads.component.scss']
})
export class CloseLeadsComponent {
  ...
  getOpenLeads(data:GetWorkListDTO) {
    this._salesleadService.getLeadWorkList(data).subscribe(res => {
      this.closedLeads = res.closedLeads
