...
@Component({
  selector: 'app-lead-in-progress',
  templateUrl: './lead-in-progress.component.html',
  styleUrls: ['./lead-in-progress.component.scss']
})
export class LeadInProgressComponent {
  ...
  getOpenLeads(data:GetWorkListDTO) {
    this._salesleadService.getLeadWorkList(data).subscribe(res => {
      this.LeadData = res.workInProgress
